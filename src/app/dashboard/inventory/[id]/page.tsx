      ...
      <Card className="mt-6">
        ...
        <CardContent className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="text-slate-900">${product.cost.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-slate-500">Unit price</p>
            <p className="text-slate-900">${product.unitPrice.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-slate-500">Stock quantity</p>
            <p className="text-slate-900">{product.stockQty}</p>
          </div>
          <div>
            <p className="text-slate-500">Reorder level</p>
            <p className="text-slate-900">{product.reorderLevel}</p>
          </div>
          {product.description && (
            <div className="col-span-2">
              <p className="text-slate-500">Description</p>
              <p className="whitespace-pre-wrap text-slate-900">{product.description}</p>
            </div>
          )}
        </CardContent>
      </Card>
